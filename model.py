...
def predict_labels(images, model_options, image_pyramid=None,skips=0, logits_required=True):
  """Predicts segmentation labels.
      ...
      with tf.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
        ...
        for output_stride in decoder_output_stride:
          ...
          for i, name in enumerate(feature_list):
            ...
            if skips==1:
                #print(skips)
                dim, width, height, n_feat_map = end_points[feature_name].get_shape().as_list()

                fc = tf.get_variable("FC_{}".format(i), shape=[n_feat_map, 256],  # (?, 25921, 256)
                                     initializer=tf.random_normal_initializer(0., 0.005))

                embed = tf.reshape(end_points[feature_name], [-1, n_feat_map])
                end_points[feature_name] = tf.matmul(embed, fc)
                end_points[feature_name] = tf.reshape(end_points[feature_name], [-1, width, height, 256])
