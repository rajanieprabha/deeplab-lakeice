...
flags.DEFINE_string('vis_logdir', '/home/pf/pfshare/data/MA_Rajanie/models/research/deeplab/datasets/lake/exp/train_on_trainval_set/vis', 'Where to write the event logs.')

flags.DEFINE_string('checkpoint_dir', '/home/pf/pfshare/data/MA_Rajanie/models/research/deeplab/datasets/lake/exp/train_on_trainval_set/train', 'Directory of model checkpoints.')
...
flags.DEFINE_list('vis_crop_size', '481,641',
...
flags.DEFINE_string('dataset_dir', '/home/pf/pfshare/data/MA_Rajanie/models/research/deeplab/datasets/lake/tfrecord', 'Where the dataset reside.')

flags.DEFINE_enum('colormap_type', 'lake', ['pascal', 'cityscapes', 'lake'],
...
flags.DEFINE_string('logits_file', None, 'Where to save logits.')
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
                   ...
                   raw_save_dir,logits,  train_id_to_eval_id=None):
  ...
  (original_images,
   ...
   image_names,
   image_heights,
   image_widths, logits) = sess.run([original_images, semantic_predictions,
                             image_names, image_heights, image_widths,logits])
  ...
  for i in range(num_image):
    ...
    # Save prediction.
    save_annotation.save_annotation(
        crop_semantic_prediction, save_dir, "%s_pred" % (image_names[i]),
        add_colormap=True,
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    #logits = tf.reshape(logits, shape=(-1,5))
